 ...
 * @since 2023/12/11
 */
public class EchoAnnotationApplicationContext implements ApplicationContext {
	...
	public EchoAnnotationApplicationContext(String basePackage) {
		try {
			// com.lxcecho
			//1 把.替换成\
			...
			for (File child : childrenFiles) {
				...
				if (child.isDirectory()) {
					...
					if (pathWithClass.contains(".class")) {
						...
						//4.6 判断类上面是否有注解 @EchoBean，如果有实例化过程
						//4.6.1 获取类的Class
						Class<?> clazz = Class.forName(allName);
						//4.6.2 判断不是接口
						if (!clazz.isInterface()) {
							//4.6.3 判断类上面是否有注解 @EchoBean
							EchoBean annotation = clazz.getAnnotation(EchoBean.class);
							if (annotation != null) {
								//4.6.4 实例化
	...
	private void loadDi() {
		...
		for (Map.Entry<Class<?>, Object> entry : entries) {
			...
			for (Field field : declaredFields) {
				//4 判断属性上面是否有@Di注解
				EchoDI annotation = field.getAnnotation(EchoDI.class);
				if (annotation != null) {
					//如果私有属性，设置可以设置值
